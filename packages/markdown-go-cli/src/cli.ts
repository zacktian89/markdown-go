...
const webRoot = path.join(distDir, "web");
const themesDir = path.join(distDir, "theme");
...
const packageVersion = fs.existsSync(packageJsonPath)
  ? JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")).version ?? "0.0.0"
  : "0.0.0";
...
async function main() {
  ...
  const customThemes = await loadThemes();
  ...
  const server = http.createServer(async (request: IncomingMessage, response: ServerResponse) => {
    ...
    const staticFilePath = resolveStaticPath(urlPath);
